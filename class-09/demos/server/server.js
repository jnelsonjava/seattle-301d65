...
function sendRestaurantData(request, response){
  ...
  const urlToSearch = `https://developers.zomato.com/api/v2.1/search?lat=${request.query.latitude}&lon=${request.query.longitude}&start=${request.query.page * 20}`;
  ...
  superagent.get(urlToSearch)
    ...
    .then(resultFromZomato => {
      ...
      const arrFromJson = jsonObj.restaurants;
      const newArr = [];
